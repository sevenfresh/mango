...
export class ComponentProvider implements vscode.TreeDataProvider<Dependency> {
    ...
    if (this.pathExists(packageJsonPath)) {
      ...
      const toDep = (moduleName: string, version: string): Dependency => {
        if (
          this.pathExists(
            path.join(
              this.workspaceFolders ? this.workspaceFolders[0].name || "" : "",
              "node_modules",
              moduleName
            )
          )
